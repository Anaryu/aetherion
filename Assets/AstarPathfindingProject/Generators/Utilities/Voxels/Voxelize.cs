...
public class Voxelize {
	...
	public void BuildPolyMesh (VoxelContourSet cset, int nvp, out VoxelMesh mesh) {
		...
		{
			Debug.LogError ("To many vertices - Couldn't generate mesh");
			mesh = new VoxelMesh ();
			//yield break;
			return;
		}
	...
	public void BuildRegions () {
		...
		//FilterSmallRegions (srcReg,10, voxelArea.maxRegions);
	...
	public ushort[] ExpandRegions (int maxIterations, uint level, ushort[] srcReg, ushort[] srcDist, ushort[] dstReg, ushort[] dstDist, List<int> stack) {
		...
		//If switched an odd number of times, switch memory
		/*if (oddeven % 2 != 0) {
	...
	/*public void RasterizeTri (Vector3 p1, Vector3 p2, Vector3 p3, Vector3 bMin, Vector3 bMax, float cs) {
		
		float[] vIn = new float[7*3];
		float[] vOut = new float[7*3];
		float[] vInRow = new float[7*3];
		
		int x0 = Mathf.RoundToInt (Utility.Min (p1.x,p2.x,p3.x));
		int x1 = Mathf.RoundToInt (Utility.Max (p1.x,p2.x,p3.x));
		
		int z0 = Mathf.RoundToInt (Utility.Min (p1.z,p2.z,p3.z));
		int z1 = Mathf.RoundToInt (Utility.Max (p1.z,p2.z,p3.z));
		
		x0 = Mathf.Clamp (c0,0,bMax
		for (int z=z0;z<=z1;z++) {
			
			Utility.InsertVector (vIn,0*3,p1);
			Utility.InsertVector (vIn,1*3,p2);
			Utility.InsertVector (vIn,2*3,p3);
		
			float cz = z;
			
			int nvrow = 3;
			
			nvrow = Utility.ClipPoly (vIn,nvrow,vOut,0,1,-cz);
			
			if (nvrow < 3) {
				//Debug.DrawLine (new Vector3 (-10,0,z),new Vector3(10,0,z),Color.red);
				debugString += nvrow +" a\n";
				continue;
			}
			nvrow = Utility.ClipPoly (vOut,nvrow,vInRow,0,-1,cz+1);
			
			Color c = nvrow < 3 ? Color.magenta : Color.blue;
			...
			Debug.DrawLine (new Vector3 (-10,0,z),new Vector3(10,0,z),Color.green);
			...
			for (int x=x0;x<=x1;x++) {
				...
				nv = Utility.ClipPoly (vInRow, nv, vOut, 1, 0, -cx);
				
				if (nv < 3) {
					//Debug.DrawLine (new Vector3 (x,0,-10),new Vector3(x,0,10),Color.red);
					continue;
				}
				
				nv = Utility.ClipPoly (vOut, nv, vIn, -1, 0, cx+1);
				
				//if (nv < 3) {
				//	continue;
				
				c = nv < 3 ? Color.magenta : Color.blue;
				
				//Debug.DrawLine (new Vector3 (x,0,-10),new Vector3(x,0,10),c);
				...
				float smax = vIn[1];
				for (int i = 1; i < nv; ++i) {
					smin = Mathf.Min (smin, vIn[i*3+1]);
					smax = Mathf.Max (smax, vIn[i*3+1]);
				}
				
				debugString += "Min "+smin+" Max "+smax+"\n";
				Debug.DrawLine (new Vector3 (x,smin,z),new Vector3(x,smax,z),Color.cyan);
			}
		}
	}*/
}
